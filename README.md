...
인프라의 중요성: AWS EC2를 직접 세팅하면서 리눅스 권한 설정, 방화벽(보안 그룹) 등 개발 외적인 인프라 지식이 필요함을 깨달음.

자동화의 편리함: GitHub Actions를 도입하여 코드 한 줄 수정으로 배포가 끝나는 경험을 통해 개발 생산성에 얼마나 큰 영향을 미치는지 체감 하게 됨.

협업의 필요성: 1인 개발을 진행하며 작은 오타나 설정 오류를 찾는 데 많은 시간을 쏟음. 이를 통해 코드 리뷰와 동료와의 소통이 문제 해결에 큰 도움이 될것이라 생각함.
